  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sX GET https://download.pydio.com/pub/linux/debian/dists/bionic/main/binary-amd64/Packages |grep -A 7 -m 1 'Package: pydio-core' | awk -F ': ' '/Version/{print $2;exit}' | sed 's/..$//' ''',
            returnStdout: true).trim()
